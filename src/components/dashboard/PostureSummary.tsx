...
const PostureSummary: React.FC<PostureSummaryProps> = ({ posture }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      {/* Score */}
      <Card className="flex items-center gap-5 col-span-1" padding="lg">
        {/* Ring */}
        <div className="relative flex-shrink-0" aria-hidden="true">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className={clsx('text-2xl font-bold leading-none', scoreColor)}>{posture.score}</span>
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Security Posture</span>
          </div>
          <h1 className={clsx('text-lg font-bold', scoreColor)} aria-label={`Security posture: ${posture.score} out of 100 — ${posture.label}`}>
            {posture.label}
          </h1>
          ...
          <p className="text-2xs text-gray-400 mt-2 max-w-[160px]">
            Score reflects asset risk, finding severity, exposure, and data completeness.
          </p>
        </div>
      </Card>
      ...
      <div className="col-span-1 lg:col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[
          ...
          },
          {
            ...
            bgAccent: 'bg-red-50 border-red-100',
          },
          {
            label: 'Open Findings',
            value: posture.openFindings,
            sub: '12 critical, 25 high',
            icon: <AlertOctagon className="w-4 h-4" />,
            color: 'text-orange-700',
            iconColor: 'text-orange-400',
            bgAccent: 'bg-orange-50 border-orange-100',
          },
          {
          ...
          },
          {
            label: 'Monitoring',
            value: '96%',
            sub: '1 sensor degraded',
            icon: <Eye className="w-4 h-4" />,
            color: 'text-yellow-700',
            iconColor: 'text-yellow-500',
            bgAccent: 'bg-yellow-50 border-yellow-100',
          },
        ].map((kpi, i) => (
          <div
            key={i}
            className={clsx(
              'rounded-lg border p-3 flex flex-col gap-1 bg-white shadow-card',
              kpi.bgAccent || 'border-surface-border',
            )}
          >
            <div className={clsx('flex items-center gap-1.5', kpi.iconColor)}>
              {kpi.icon}
              <span className="text-2xs font-semibold text-gray-500 uppercase tracking-wider">{kpi.label}</span>
            </div>
            <div className={clsx('text-2xl font-bold', kpi.color)}>{kpi.value}</div>
            <div className="text-2xs text-gray-400">{kpi.sub}</div>
          </div>
        ))}
      </div>
